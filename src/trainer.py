            ...
            if global_step % args.gradient_accumulation_steps == 0:
                ...
                if args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.evaluate_during_training:
                        ...
                        logger.info(f"epoch: {epoch}, step: {global_step}, eval_loss: {eval_loss:.4f}, acc: {eval_acc:.4f}, progress: {progress:.2f}")
                        
                        preds = (outputs[0].squeeze(-1) > 0.5)
                        preds_ids = (preds*input_ids).tolist()
                        preds_tokens = tokenizer.batch_decode(preds_ids, skip_special_tokens=True)
                        logger.info(f"INFER: {' '.join(preds_tokens)}")
                        model.train()
